...
describe("Satellites", () => {
    ...
    it("should paginate for 2 satellites", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/satellites?page=1&amount=2")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should retrieve a satellite by ID", async () => {
        const res = await chai
            .request(app)
            .get(`/api/v1.2/satellites/${satelliteId}`)
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should filter satellites by name", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/satellites?name=Moon")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should filter satellites by type", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/satellites?type=MOON")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should sort satellites by name", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/satellites?sortBy=name")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should reject non-numeric diameter during update", async () => {
        const res = await chai
            .request(app)
            .put(`/api/v1.2/satellites/${satelliteId}`)
            .set("Authorization", `Bearer ${token}`)
            .send({
                name: "Phobos",
                age: 4.5e9,
    ...
    it("should update one satellite field", async () => {
        const res = await chai
            .request(app)
            .patch(`/api/v1.2/satellites/${satelliteId}`)
            .set("Authorization", `Bearer ${token}`)
            .send({
                name: "Updated Moon"
            });
    ...
    it("should delete a satellite by ID", async () => {
        const res = await chai
            .request(app)
            .delete(`/api/v1.2/satellites/${satelliteId}`)
            .set("Authorization", `Bearer ${token}`);
