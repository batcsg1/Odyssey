...
describe("Stars", () => {
    ...
    it("should paginate for 2 stars", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/stars?page=1&amount=2")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should filter stars by name", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/stars?name=Betelgeuse")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should filter stars by type", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/stars?type=MAIN_SEQUENCE")
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should sort stars by name", async () => {
        const res = await chai
            .request(app)
            .get("/api/v1.2/stars?sortBy=name")
            .set("Authorization", `Bearer ${token}`);
