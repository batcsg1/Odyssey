...
describe("Super Admin", () => {
    ...
    it("should retrieve a super admin by id ", async () => {
        const res = await chai
            .request(app)
            .get(`/api/v1.2/users/${anotherSuperAdminUserID}`)
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should update an admin user's data", async () => {
        const res = await chai
            .request(app)
            .patch(`/api/v1.2/users/${adminUser.id}`)
            .set("Authorization", `Bearer ${token}`)
            .send({
                firstName: "Patrick"
            });
    ...
    it("should reject a super admin user updating another super admin user", async () => {
        const res = await chai
            .request(app)
            .patch(`/api/v1.2/users/${anotherSuperAdminUserID}`)
            .set("Authorization", `Bearer ${token}`)
            .send({
                firstName: "Gen"
            });
    ...
    it("should reject deleting a super-admin user", async () => {
        const res = await chai
            .request(app)
            .delete(`/api/v1.2/users/${anotherSuperAdminUserID}`)
            .set("Authorization", `Bearer ${token}`);
    ...
    it("should delete an admin user", async () => {
        const res = await chai
            .request(app)
            .delete(`/api/v1.2/users/${adminUser.id}`)
            .set("Authorization", `Bearer ${token}`);
